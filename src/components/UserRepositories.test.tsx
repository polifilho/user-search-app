...
describe('UserRepositories Component', () => {
    ...
    test('should display the message "repositories not found" when repository is 0', () => {
      ...
      expect(screen.getByText(/Sorry! Repositories not found from this user./i)).toBeInTheDocument();
    });
  
    test('should render only 1 item about the first user', () => {
      const reposMock = [
        { userName: 'testUser', loadMore: false, repositories: [{ name: 'Repo 1', description: '', stargazers_count: 0 }] },
      ];
      render(<UserRepositories {...defaultProps} repos={reposMock} />);
    ...
    test('should render 20 itens and the button to load more should render', () => {
      const reposMock = [
        { userName: 'testUser', repositories: Array.from({ length: 20 }, (_, i) => ({ name: `Repo ${i}`, description: '', stargazers_count: 0 })), loadMore: true },
      ];
      render(<UserRepositories {...defaultProps} repos={reposMock} />);
